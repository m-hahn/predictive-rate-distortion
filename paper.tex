...
Statistical complexity can be computed easiy if the $\epsilon$-machine is analytically known, but estimating statistical complexity from time series is very challenging and appears to at least require additional assumptions about the process~\citep{clarke2003application}.
...
As in OCE (Section~\ref{sec:oce}), we assume that sample trajectories of length $2M$ are available and we aim to compress pasts of length $M$ to predict futures of length $M$:
\begin{equation}\label{eq:ib-oce-repeat}
	\max_{Z} \left( \operatorname{I}[X_{-M\dots -1}, Z] - \lambda \cdot \operatorname{I}[Z, X_{0\dots M-1}] \right)
\end{equation}
where $Z$ ranges over all random variables such that the Markov condition (REF) holds:
\begin{equation}
    Z\bot X_{0\dots M-1} | X_{-M\dots -1}
\end{equation}

When deploying such a predictive code $Z$, two components are required: a distribution $P(Z|X_{-M\dots -1})$ that \emph{encodes} past observations into a code $Z$, and a distribution $P(X_{0\dots M-1}|Z)$ that \emph{decodes} the code $Z$ into predictions about the future.

Let us assume that an \emph{encoder} $\phi$ is given, expressed in some family of function approximators, which stochastically transforms observation sequences $x_{-M\dots -1}$ into a random code $Z \in \mathbb{R}^N$: 
\begin{equation}
Z = \phi(x_{-M\dots -1})
\end{equation}
distributed according to $Z \sim P_\phi(Z|x_{-M\dots -1})$.
From this, one can obtain the decoding distribution over future observations via Bayes' rule: %, using the Markov condition: 
\begin{equation}
	\label{eq:cond-fut}
	\begin{split}
...
However, neither of the two expectations in the last line of~(\ref{eq:cond-fut}) is tractable, as they require summation over exponentially many sequences.
...
\begin{equation}
q(Z) \text{ approximates } P(Z) = \E_{X_{-M \dots -1}} \phi(Z|X_{-M \dots -1})
\end{equation}
...
\end{equation}
for each code $Z \in \mathbb{R}^N$.

If we fix a stochastic process $(X_t)_t$ and an encoder $\phi$, then the following two bounds hold for \emph{any} choice of the decoder $\psi$ and the distribution $q$:
...
\begin{equation}\label{eq:prd-recall}
	\min_{Z : Z \bot X_{0\dots M} | X_{-M\dots-1}}	\left[H[X_{0\dots M} | Z] + \lambda I[Z, X_{-M\dots-1}]\right]
...
\end{equation} % should be P_\psi (X_{0...M}|z)? % done (mhahn)
where $\phi, \psi, q$ range over all triples of the appropriate types described above. From a solution to (\ref{eq:bound}), one obtains a solution to (\ref{eq:prd-recall}) by setting $Z$ to be the code generated by the encoder $\phi$.
...
\begin{equation}\label{eq:eq1-new}
 I[Z, X_{-M\dots-1}] =   \mathbb{E}_{X}\left[ \operatorname{D_{KL}}\infdivx{P(Z|X)}{q(Z)}\right]
\end{equation}
...
\end{equation}
If $\phi, \psi, q$ are not exact solutions to (\ref{eq:bound}), the two propositions guarantee that we still have upper bounds on rate and distortion for the code $Z$ generated by $\phi$:
...
TODO how is the input space encoded, embeddings
...
We optimize the parameters of the neural networks expressing $\phi, \psi, q$ for (\ref{eq:bound}) using Backpropagation and Adam \citep{kingma-adam:-2014}, a standard and widely used gradient descent-based method for optimizing neural networks.
During optimization, we approximate the gradient by taking a single sample from $Z$ (\ref{eq:z-gauss}) per sample trajectory $x_{-M}, \dots, x_M$ and use the reparametrized gradient estimator introduced by \citet{kingma-auto-encoding-2014}.
...
After optimizing the parameters on a set of observed trajectories, we estimate rate and distortion on the test set.
Given parameters for $\phi, \psi, q$, we evaluate the PRD objective~(\ref{eq:bound}), rate (\ref{eq:bound1-new}), and distortion (\ref{eq:bound2-new}) on the test set by taking, for each timeseries $X_{-M}...X_M$ from the test set, a single sample $z  \sim \mathcal{N}(\mu, \sigma^2)$ and computing Monte Carlo estimates for rate
\begin{equation}\label{eq:bound-mc-rate}
\mathbb{E}_{X}\left[ \operatorname{D_{KL}}\infdivx{P(Z|X)}{q(Z)}\right] \approx	\frac{1}{N}	\sum_{X_{-M...M} \in TestData}  \log \frac{p_{\mathcal{N}(\mu, \sigma^2)}(z)}{q(z)}
\end{equation}
where $p_{\mathcal{N}(\mu, \sigma^2)}(z)$ is the Gaussian density, and distortion
\begin{equation}\label{eq:bound-mc-distortion}
 -	\mathbb{E}_{z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M} | z)\right] \approx \frac{1}{N}	\sum_{X_{-M...M} \in TestData}	\log P(X_{0\dots M} | z_{-M...-1})
\end{equation}
...
Thanks to (\ref{eq:bound1-new}-\ref{eq:bound2-new}), these estimates are guaranteed to be \emph{upper bounds} on the true rate and distortion achieved by the code $Z  \sim \mathcal{N}(\mu, \sigma^2)$, up to sampling error introduced into the Monte Carlo estimation by sampling $z$ and the finite size of the test set.
...
Given that NPRD \emph{provably} provides \emph{upper bounds} on the predictive rate-distortion curve (up to sampling error), one can objectively compare the quality of different estimation methods:
...
\end{equation}
which is lower bounded by $T$ times the entropy rate: $R \approx T \cdot \operatorname{H}[X_0|\overleftarrow{X}]$, and thus is unbounded, assuming that the entropy rate is nonzero.
...
We have applied NPRD to the problem of predicting English on the level of part-of-speech tags in Section~\ref{sec:pos}.
