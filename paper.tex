...
%We present our approach to estimating the memory-surprisal tradeoff with general-purpose function approximators.
%As in OCE, we fix a horizon $T$.
%Unlike OCE, computational costs scale linearly with M, which will enable application to longer-range dependencies.
%We are given a past $X_{-T\dots -1}$ and a future $X_{0\dots T-1}$.


\subsection{Bound on Predictive Rate-Distortion}
%Our method will be based on a bound.
%Our estimator uses a diagonal Gaussian $Z \sim N(\mu, \sigma^2)$
%iOur estimator is centered around three objects: % parameterized by parameters $\theta$:
...
%We will write the objective in (\ref{eq:ib}) in a way amenable to gradient-based optimization within a parametric family of function approximators.

Let us assume that an \emph{encoder} $\phi$ is given, expressed in a family of function approximators. % and \emph{decoder} $\psi$ 
...
%%We will be in a setting where neither of the two mutual information terms can be computed exactly.
%Instead, we will find a way to bound both of them using approximations to the Bayes-optimal decoder and to the marginal of $Z$:
%The \emph{decoder} $\psi$ transforms each vector $z \in \mathbb{R}^N$ into a probability distribution over sequences $x_{0\dots T-1}$.
%$\phi$ and $\psi$ act as encoders and decoders to and from the latent code space $\mathbb{R}^N$.
%The third component is a probability distribution $q$ over $\mathbb{R}^N$.
%Further, let $\psi$ be a \emph{decoder}, which is any map from the points in $\mathbb{R}^N$ into probability distributions over sequences $x_{0\dots T-1}$.
%Here, there need $\psi$ need not be the `correct' decoder for $\phi$.
%Finally, let $q$ be any distribution over $\mathbb{R}^N$.
...
%It is in this sense that $\psi$ `approximates' the conditional $P(X_{0\dots M-1}|Z)$.
\end{proposition}
...
%These inequalities are tight if $\psi$ is the Bayes optimal decoder ($\psi_z(y) = \sum_{x} \phi(z|x) P(x, y)$), and $q$ is the marginal of $Z$.
% N(\mu(x), \sigma^2(x))
%Note that, for finite $M$, the predictive rate-distortion objective is equivalent to
...
%As this optimization ranges over all distributions $\phi, \psi, q$ of the appropriate domains, t
...
%Optimizing them for the objective (\ref{eq:bound}) will result in functions that approximate the optimal decoder and the marginal.
...
%\paragraph{Why introduce $\psi, q$?} Ideally, $\psi, q$ are the Bayes optimal decoder and the marginal. Why not directly set them to these values?
%The reason is that neither of these are computationally tractable if $\phi$ comes from a powerful family of function approximators such as recurrent neural networks.
%Instead, we introduce these as separate functions that are parameterized within the family of approximators.
%Optimizing them for the objective (\ref{eq:bound}) will result in functions that approximate the optimal decoder and the marginal.
...
%By choosing sufficiently powerful families, the bound in \ref{eq:bound}

%Ideally, $\psi$ would be equal to the optimal decoder appropriate to $\phi$ ($\psi_z(y) = \sum_{x} \phi(z|x) P(x, y)$), but we will be in a situation where calculting this optimal decoder is not tractable and we will instead create a variational approximation within the family of function approximators.
...
%where $softmax(x)_i := \frac{\exp(x_i)}{\sum_{j \in S} \exp(x_j)}$
%where $S$ is the state space in which the process $(X_t)_t$ takes its values.


%The implementation follows the standard \emph{sequence-to-sequence} methodology, which we define here for self-containedness.
%$\phi$ was computed as follows:
%$h_{-T} = 0 \in \mathbb{R}^k$,
%$h_t = f(x_t, h_{t-1})$
%for $t=-T+1, ..., 0$, where $f$ is the LSTM cell, define below, and then 
%
%$\psi$ was computed as follows:
%$g_0 := $ a single sample from $Z$,
%$g_t := f(x_t, g_{t-1})$ for $t=0, ..., T-1$,
%and $\psi(x_t|x_{1...t-1}) = softmax(W_o g_t)$, where $softmax(x)_i := \frac{\exp(x_i)}{...}$.
...
%The literature on Variational Autoencoders has suggested a couple of choices for $q$: (1) $N(0,1)$, (2) various kinds of normalizing flows \citep{rezende-variational-2015, kingma-improving-2016, papamakarios-masked-2017} that transform $N(0,1)$ to obtain a more complex $q$, (3) learnable mixtures of Gaussians, (4) combinations of 3. and 4. -- that is, flows that act on a mixture of Gaussians \citep{papamakarios-masked-2017}.
%So far I implemented (1-3) using affine inverse autoregressive flows grounding in a learnable mixture of Gaussians with autoregressive weights, both computed using MADE \citep{germain-made:-2015}, as in \cite{papamakarios-masked-2017}.
%
%The most general method to date seems to be \citep{huang-neural-2018}, which provides a universal approximator to distributions.
%


%TODO define
%We require a parametric family of distributions that allow fast computation and differentiation of the probability density.


%With these choices of function approximators, the bound is tight in the limit of sufficiently large neural networks:
%\begin{proposition}
%	Let $(X_t)_t$ be a finite-order Markov process with finite state space. Choose $\beta \in [0,1]$.
%	
%%	Assume that the PRD objective is solved y 
%	
%	
%Then, for any $\epsilon > 0$, there are horizons $T$, dimensionalities $k$, layer sizes $l$, and flow lengths $m$ such that 
%	the optimum is no more than $\epsilon$ worse than the nonparametric optimum.
%
%%In the limit of sufficiently large neural networks, the optimum of ... is equal to ...
%\end{proposition}
%
%\begin{proof}
%Let $Z$ be a solution for
%\begin{equation}\label{eq:ib}
%	\max_{Z : Z \rightarrow X_{< 0} \rightarrow \overrightarrow{X}} \left( I[\overrightarrow{X}, Z] - \beta \cdot I[Z, (\overleftarrow{X})] \right)
%\end{equation}
%By assumptions, $Z$ takes values in a finite set.
%	Using some arbitrary injective mapping, we will think of $Z$ as taking values in $\mathbb{R}^n$.
%	We then smooth $Z$ by adding $N(0,\epsilon/100)$.
%
%
%	Let $\psi'$ be a recurrent network that approximates the map from $Z$ to $P(\overrightarrow{X}|Z)$ with loss $\epsilon/4$.
%	Let $p'$ be a Neural Autoregressive Flow approximating the marginal $P(Z)$ up to $\epsilon/4$: $|p(Z \in A) - p'(Z \in A)| < \epsilon/4$ for all $A$.
%
%
%
...
%	\partial_{\phi, \psi, q}	\log P(X_{0\dots T} | z_{-M...0}) + \beta \cdot \log \frac{N(\mu, \sigma^2)(z)}{q(z_{-M...0})}
%\end{equation}
%for a given training example $x_{-M...M}$.
...
%The use of a held-out set is motivated by the fact that -- as for OCE -- the codebook may have overfitted to the set used for optimization.


%
%\paragraph{Other Choices of $q$}
%The literature on Variational Autoencoders has suggested a couple of choices for $q$: (1) $N(0,1)$, (2) various kinds of normalizing flows \citep{rezende-variational-2015, kingma-improving-2016, papamakarios-masked-2017} that transform $N(0,1)$ to obtain a more complex $q$, (3) learnable mixtures of Gaussians, (4) combinations of 3. and 4. -- that is, flows that act on a mixture of Gaussians \citep{papamakarios-masked-2017}.
%So far I implemented (1-3) using affine inverse autoregressive flows grounding in a learnable mixture of Gaussians with autoregressive weights, both computed using MADE \citep{germain-made:-2015}, as in \cite{papamakarios-masked-2017}.
%
%The most general method to date seems to be \citep{huang-neural-2018}, which provides a universal approximator to distributions.
%
%%https://arxiv.org/pdf/1810.03256.pdf
%
%
%%%If the LSTM is capable of doing compression well (it should be able to do so if it has enough capacity), the main limitation to using this formulation is the choice of $q$: The bound is tight if $q$ is actually the marginal distribution of $Z$, and gets worse (overestimating memory demand) if $q$ cannot approximate the marginal distribution well.
%%For instance, a unimodal $q$ may give a bad fit if the process has a set of discrete states that should not be `confused' by $Z$. %giving $q$ the capability of accomodating several modes may be 
%
%
%
...
%\paragraph{Why would it work?}
%Neural networks as universal function approximators.
%any discrete codebook can be emulated to arbitrary precision with sufficiently large neural networks.
%similarly continuous codebooks (e.g. Gaussian codes) can be emulated.
%neural autoregressive flows proven to be universal approximators of distributions, so can emulate the `real' $q$ closely.
%learnability question is more difficult and subject of ongoing research, but recent results provide some learnability guarantees.
...
%This formulation is the same as the Deep Variational Information Bottleneck \cite{alemi-deep-2016} (but it seems this RNN setting hasn't appeared before).
...
%(\ref{eq:bound}) is also related to the objective function of the Variational Lossy Autoencoder of \cite{chen-variational-2016}, which 


%If the family is sufficiently powerful, the bound 
%For $\phi$, we consider recurrent neural networks that 

%
%
%\begin{enumerate}
%\item an neural network computes $\mu, \sigma^2 \in \mathbb{R}^N$ from the past $X_{-T\dots -1}$
%\item sample $z \sim N(\mu, \sigma^2)$
%\item feed $z$ as the initial state of a neural network that predicts the future $X_{0\dots T}$
%\end{enumerate}
%
%The components are trained to optimize the following loss funtion: % (\ref{eq:ib}):
%%We define, with a tradeoff parameter $\beta$, the loss function
%\begin{equation}\label{eq:bound}
%	-	\mathbb{E}_{z \sim N(\mu, \sigma^2)}\left[\log P(X_{0\dots T} | z)\right] + \beta \cdot D_{KL}(N(\mu, \sigma^2) || q)
%\end{equation}
%where $\mu, \sigma^2$ are computed from $X_{-T\dots -1}$ in (1), and $P$ is computed in (3), and $q$ is a (potentially parameterized) probability distribution. 
%The LSTMs (and $q$, if it is parameterized) are together optimized for this loss function.
%
%
%\paragraph{Relation between (\ref{eq:bound}) and (\ref{eq:ib})}
%The first term in (\ref{eq:bound}) is an upper bound on $H[X_{0\dots T}|Z]$ (future surprisal), the second term is an upper bound on $I[Z, (X_{< 0})]$ (memory cost).
%In total, (\ref{eq:bound}) is an upper bound on the loss function
%\begin{equation}\label{eq:obj}
%H[X_{0\dots T}|Z] + \beta \cdot I[Z, (X_{-T\dots -1})]
%\end{equation}
%which is (up to finiteness of the horizon) equivalent to the objective in (\ref{eq:ib}).
%%(I think one would say that \ref{eq:bound} is a \emph{variational} upper bound on \ref{eq:obj}.)
%With sufficiently powerful RNNs and a flexible family for $q$, optimization could theoretically make this bound tight.
%
%\begin{proposition}
%In the limit of sufficiently large neural networks, the optimum of ... is equal to ...
%\end{proposition}
%%As explained above, varying $\beta$, the optima for this objective function define the memory-surprisal tradeoff curve.
%%To approximate this optimization, we can use backpropagation to optimize \ref{eq:bound}.
%
%The solutions obtained from optimizing (\ref{eq:bound}) will always constitute an upper bound (overestimating memory need at a given surprisal level) on the actual solutions of (\ref{eq:obj}).
%Note that the same is true when applying the AB algorithm, which is also guaranteed to find an upper bound.
%While the approximation is not guaranteed to be good, I think there is value in having a theoretical guarantee that the result is always an upper bound.\footnote{E.g., we can see which estimation method gives the best fit by seeing which one gives the lowest memory values.}
%
...
%For OCF, we used $M=1,2...$ until the number of observed sequences was greater than $2 \times 10^4$.

%\paragraph{OCF}


%\paragraph{Neural Method}
%Hyperparameters




%We optimize $q(z|x)$, marginal $s(z)$, and $r(y|z)$.
\paragraph{OCE}
...
$$\frac{1}{N} \sum_{i=1}^N D_{KL}(q(\cdot|x_i) || s(\cdot))$$ %\E_{z \sim q(\cdot|x_i)} \log \frac{q(z|x_i)}{s(z)} $$
and
...
%Given that all unseen futures were mapped to $\omega$, this method may slightly bias towards underestimating entropy of the 


%In our experiments, we applied this until the number of sequences was about $20,000$ (agreeing with Marzen and Crutchfield).
%Depending on the topological entropy.
...
%Their method using the (already known) epsilon machine  works well.
...
Comparing the two diagrams shows that NPRD effectively recovers the $\epsilon$-machine. \rljf{More detail on this part: it's a bit telegraphic}% even though $Z$ is a multivariate Gaussian variable. 
...
\begin{tikzpicture}[shorten >=1pt,node distance=4cm,on grid,auto] 
   ...
   \node[state, fill=blue] (C) [below right=of B] {$C$}; 
	...
	(C) edge [bend left=20] node [left] {$\frac{1}{2} | 1$} (A)
	;
%
%%          edge  node [swap] {1} (B)
%    (C) edge  node  {1} (A)
...
%NPRD, in contrasts, encodes the codebook in learned weight matrices that take less than 500MB

%
%This far exceeds what OCE can handle by several orders of magnitude, at least without further optimizations.
%
%Does NPRD 
%
%
%it necessary to increase the number of training iterations to also obtain good estimates of the rate.
%NPRD recovers codebooks with almost zero distortion, though it slightly overestimates the necessary rate.
%NPRD thus finds codebooks that effectively distinguish between millions of different pasts, while requiring orders of magnitude fewer parameters.
%
%The codebook is encoded in weight matrices, takes less than 500MB while training and less when evaluating.
%Directly applying OCE requires creating and storing all $3^{15} \approx 14M$ sequences.
...
%
%As (CITE) have noted, evaluating (X) on finite data will overestimate MI with the future.
%To address this, we estimate (X) on held-out data.
%
%We compare our estimator with OCF on (1) POS-level language modeling, (2) word-level language modeling
%
%Challenges for OCF:
%
%- held-out data will have unobserved sequences.
%We add an out-of-vocabulary pseudo-sequence to the table of observed sequences.
%
...
%(The potential number of grammatical 15-word POS sequences is far larger than the corpus -- given the cross-entropies achieved by the LSTMs, the entropy of 15-word English POS sequencs far exceeds 20 nats.)
%The OCE results show that remembering a codebook of longer and longer sequences leads to overfitting.
%Even if computational hurdles were overcome, it seems unlikely that OCE could induce such a large codebook from linguistic corpora, and still maintain generalisability to unseen data.
%How does NPRD manage to get lower distortions?
%This points to an advantage of continuous vector-valued codes, studied extensively in the literature on neural network methods for Natural Language Processing:
%Given the internal structure in code space, it can be expected that, on unseen data, they will construct codes `on the fly' even
...
\end{equation}
%if the rate-predictiveness curve is sufficiently regular at $(P,R)$.
...
%
%\begin{equation}
%P = \int \exp\left(-\frac{1}{\alpha^{1/\beta}} R^{1/\beta}\right) dR
%\end{equation}
%
%\begin{equation}
%R = \int \exp\left(-\frac{1}{\alpha^{1/\beta}} R^{1/\beta}\right) dP
%\end{equation}
%



%Pretending that $t$ is continuous, we write
%\begin{equation}
%	E = \int_1^{M/c} t f(t)
%\end{equation}
%and thus
%\begin{equation}
%	t f(M/c) = \partial E/\partial M  = \exp\left(-\frac{1}{\alpha^{1/\beta}} M^{1/\beta}\right)
%\end{equation}
%and thus
%\begin{equation}
%	f(t) = \frac{1}{t} \exp\left(-\left(\frac{c}{\alpha}\right)^{1/\beta} t^{1/\beta}\right)
%\end{equation}
%We get
%\begin{equation}
%E = \int \exp\left(-\frac{1}{\alpha^{1/\beta}} M^{1/\beta}\right) dM
%\end{equation}
%
%
...
%
%
%Why would language show this behavior?
%We will describe a proposal.
%%We will describe a proposal that leads to quantitative predictions.
%As a simple model of prediction, we consider a code $Z^{(T)}$ that stores all relevant predictive information from the past $T$ words (where $T$ depends on $\lambda$), and nothing from the words further in the past.
%The predictiveness of this code is
%\begin{equation}
%	I[X_{-T...-1}, \overrightarrow{X}]
%\end{equation}
%
%Let us make the assumption that storing the relevant information from each past word requires maintaining an amount of information about each word that is bounded away from zero:
%$$I[X_{-t}, Z^{(T)}|X_{-t+1}\dots X_{-1}] = c + g(t)$$
%for each $t = 1, ..., T$, where $c > 0$ and $g(t) \geq 0$.
%We will ignore $g(t)$ in the following.
%Then storing relevant information about the last $T$ words will lead to a rate of $\approx cT$, and predictiveness of 
%\begin{equation}
%	E = \sum_{t=1}^T t f(t)
%\end{equation}
%
%
%Let $f(t)$ be the incremental contributions of past words to predicting the next word:
%$$f(t) := I[X_t, X_0|X_1\dots X_{t-1}]$$
%(Since $H[X_0]$ is finite, $f(t)$ must go to zero fast enough to be summable, even in the idealized setting of infinite contexts.)

%$E= E_0-\exp(-M^\beta) \cdot (1+M^\beta )$
%
%$\partial E/\partial M = -(1+M^\beta) \exp(-M^\beta) (-\beta) M^{\beta-1} - \exp(-M^\beta) \beta M^{\beta-1}$
%
%$= \beta \exp(-M^\beta) \left[ -(1+M^\beta)  (-1) M^{\beta-1} - M^{\beta-1}\right]$
%
%$= \beta \exp(-M^\beta) \left[ (1+M^\beta)  M^{\beta-1} - M^{\beta-1}\right]$
%
%$= \beta \exp(-M^\beta) \left[ (M^{\beta-1}+M^\beta M^{\beta-1})  - M^{\beta-1}\right]$
%
%$= \beta \exp(-M^\beta) M^{2\beta-1}$
%
%\begin{verbatim}
%plot = ggplot(dataU, aes(x=Memory, y=log(Beta))) + geom_point()+ theme_classic()  + xlim(0, 20) + theme(legend.position="none") + geom_point(data=data.frame(x=x), aes(x=x, y=-x^0.64+(2*0.64-1)*log(x)), color="red") + ylim(NA, 0)
%\end{verbatim}
%
%Furthermore, we expect that predictability on the first future word will be
%\begin{equation}
%	\sum_{t=1}^{M/c} f(t) = \sum_{t=1}^{M/c} \frac{1}{t} \exp\left(-\left(\frac{c}{\alpha}\right)^{1/\beta} t^{1/\beta}\right) 
%\end{equation}
...
%We can write the predictiveness as 
%\begin{equation}
%	I[X_{-T...-1}, \overrightarrow{X}] = \sum_{t=1}^\infty \min(t,T) I_t
%\end{equation}
...
%OCF becomes computationally intractable on this problem
%- OCF fails even on the UD dataset, with more than $10^5$ distinct bigrams.

%\paragraph{Discussion}
%Why would language show this behavior?
%%
%Assume that the incremental contributions of past words to predicting the next word decays with a power law:
%$$I[X_t, X_0|X_1\dots X_{t-1}] := d t^{-\beta}$$
%(this can be independently verified)
%but that realizing this requires maintaining a constant amount of information about each word in $Z$:
%$$I[X_{-t}, Z|X_{-t+1}\dots X_{-1}] \approx c$$
%Then storing relevant information about the last $T$ words will lead to a rate of $\approx cT$.
%Then, distortion will be lowered by
%$$d\sum_{t=1}^T t t^{-\beta} \propto \frac{d}{\beta-1} [1-T^{-\beta+2}] \approx \frac{d}{\beta-1} [1-\left(\frac{I[Z, \overleftarrow{X}]}{c}\right)^{-\beta+2}]$$
%Reasonable fit to data.
%
%Then
%$$1/\lambda = \frac{\partial E}{\partial M} = \frac{- d (-\beta+2) (M/c)^{-\beta+1}}{(\beta-1)c}$$
%$$\log\lambda = (1-\beta) \log M + \log \frac{(\beta-1)c}{- d (-\beta+2) (1/c)^{-\beta+1}}$$
%this doesn't look right!
%
%- need $\log\lambda \rightarrow M$ to be approximately linear
%
%from this get
%$\lambda \propto \exp(\alpha M)$
%Thus
%$\partial E/\partial M = \exp(-\alpha M)$
%(so under the idea above, this would lead to exponential decay of the incremental contributions)
%Thus
%$E = C - \exp(-\alpha M)$
%
%$E = 1.3 - \frac{-\log\lambda}{(-\log\lambda-0.1)^2}$
%
%Then $\frac{\partial M}{\partial E} = \lambda \propto \exp(1/(1.3-E))$
%
%
%THEORetical: $M \propto -\ln\lambda^\alpha$ with $\alpha$ e.g. $1.1$
%From this derive others:
%$E = 1.1-\exp(0.8-0.9M^{0.6}) = 1.1-\exp(0.8+0.9\log\lambda)$
%From this obtain the incremental contributions
%
%%
%%
...
However, beyond this bound, I don't know how to exactly calculate the curve. In any case, we can prove that it has finite excess entropy and infinite complexity under very general conditions:
...
Note that, as $\mathcal{X}$ is uncountable and each of its elements has measure zero, its entropy is not strictly well-defined, and some care has to be taken here.
...
We show that one can uniquely identify elements of $\mathcal{X}$ from the induced distributions over futures.
...
For all integers $i$ and any sequence $X_1^\infty \in \mathcal{X}$, the block $x_{1...t_i}$ has log-probability at least $t_i \log \frac{\mu(0)}{2}$.
Thus
$$H[X_1...X_{t_i} | X \in \mathcal{X}] \geq  t_i \log \frac{\mu(0)}{2}$$
...
Here we want to show that, even for processes with infinite statistical complexity, the PRD curve provides fine-grained quantification of the difficulty of prediction.

If the entropy $H[X_t]$ is finite (e.g., if the state space is finite)

The PRD curve, as a function of Predictiveness, is finite on the interval from 0 to (possibly excluding) the excess entropy.

\begin{proof}
A bound on the curve is given by the (convex hull) of the pairs of $$\{(H[X_1...X_t], I[(X_1...X_t), X_{>t}]) : t \in \mathbb{N}\}$$
...
Lower bounds on statistical complexity can be constructed from finite data by estimating excess entropy and predictive rate-distortion.
...
Any confidence interval for statistical complexity that has good coverage (e.g., $95 \%$) needs to include this trivial upper bound most of time (e.g., more than $95 \%$).


\begin{proposition}
Fix a positive integer $k$, a space $S$ of observable states.
%Fix any $N$ be the number
Then no confidence interval for the statistical complexity, with coverage $\beta > 0$ (e.g., 0.95), can exclude the `trivial' upper bound $k\log S$ with probability $\gamma > 1-\beta$ (e.g., $> 0.1$) for any generating process, no matter how many datapoints have been collected.
\end{proposition}
...
One can easily extend this to see that, if all datapoints have length $\leq k$, a confidence interval can never exclude $k \log |S|$ for any nonzero coverage, irrespective of the number of datapoints.
