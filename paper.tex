...
\begin{equation}\label{eq:bound-augmented}
	\min_{\phi, \psi, q,\eta} \left[\mathbb{E}_{\finitepast, \finitefuture}	\log P_\eta(\finitepast) + \left[-	\mathbb{E}_{Z \sim \phi(\finitepast)}\left[\log P_\psi(\finitefuture | Z)\right] + \lambda \cdot \operatorname{D_{KL}} [ \infdivx{P_\phi(Z|\finitepast)}{q(Z)}\right]\right].
\end{equation} % should be P_\psi (X_{0...M}|z)? % done (mhahn)
