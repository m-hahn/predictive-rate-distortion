...
* we should make sure not to use ``this'', ``that'', ``which'', etc. as bare NPs. Copyeditors don't like this.
}
...
First, it replaces semi-infinite pasts and futures with bounded-length contexts $(X_{-M \dots -1}), (\finitefuture)$:
\begin{equation}\label{eq:ib-oce}
	\max_{Z} \left( \operatorname{I}[\finitepast, Z] - \lambda \cdot \operatorname{I}[Z, \finitefuture] \right).
\end{equation}
Second, it estimates information measures directly from the observed counts of $(\finitepast), (\finitefuture)$ using the plug-in estimator of mutual information. %\rljf{Should be \dots instead of commas?}
...
\begin{equation}\label{eq:ib-oce-repeat}
	\max_{Z : Z \bot \finitefuture | X_{-M\dots-1}} \left( \operatorname{I}[Z, \finitefuture] - \lambda \cdot \operatorname{I}[\finitepast, Z] \right).
\end{equation}
...
\begin{aligned}
	\mathbb{E}_{X_{-M\dots-1}}\left[ \operatorname{D_{KL}}\infdivx{P_\phi(Z|X_{-M\dots-1})}{q(Z)}\right] &=   \mathbb{E}_{X_{-M\dots-1}} \mathbb{E}_{Z|X_{-M\dots-1}} \log \frac{P_\phi(Z|\finitepast)}{q(Z)}  \\
	& \geq  \mathbb{E}_{X_{-M\dots-1}} \mathbb{E}_{Z|X_{-M\dots-1}} \log \frac{P_\phi(Z|\finitepast)}{P(Z)}  \\
...
\begin{equation}\label{eq:bound1-new}
 \operatorname{I}[Z, X_{-M\dots-1}] \leq   \mathbb{E}_{X}\left[ \operatorname{D_{KL}}\infdivx{P_\phi(Z|\finitepast)}{q(Z)}\right]
\end{equation}
...
\paragraph{OCE} As discussed in Section~\ref{sec:oce}, OCE is affected by overfitting, %overestimates structure in the process due to overfitting, %resulting in biased estimates of rate and distortion.More specifically, 
...
It has infinite Markov order, and \citet{marzen-predictive-2016} find that OCE (at $M=5$) performs poorly. %, overestimating  even at $M=5$.
...
Following \citet{marzen-predictive-2016}, we ran OCE for $M=1,...,5$.
...
As $M \rightarrow \infty$, (\ref{eq:sol}) behaves as
\begin{equation}
...
Our analysis of PRD curves for natural language suggests that it is characterized by very high statistical complexity beyond its excess entropy.

\citet{dkebowski2018natural} argues that the excess entropy of connected texts in natural language is unbounded, meaning that the statistical complexity is unbounded. Our result suggests that the statistical complexity within individual sentences is unbounded (though the excess entropy is finite). Together, these results  
