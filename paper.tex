...
\newcommand{\finitepast}{X_{-M\dots -1}}
...
Predictive Rate-Distortion is closely related to Statistical Complexity and the $\epsilon$-machine \citep{crutchfield-inferring-1989,Grassberger1986}. %\rljf{cite Grassberger 1986}
...
\end{equation}
Second, it estimates information measures directly from the observed counts of $(X_{-M\dots -1}), (\finitefuture)$ using the plug-in estimator of mutual information. %\rljf{Should be \dots instead of commas?}
...
%\begin{equation}
%    Z\bot \finitefuture | X_{-M\dots -1}
%\end{equation}
...
\begin{equation}\label{eq:eq2-new}
  ...
  \operatorname{H}[\finitefuture|Z] =   - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(\finitefuture | Z)\right]. %H[\finitefuture|X_{-M\dots-1}] % \operatorname{H}[\finitefuture]
\end{equation}
...
\begin{equation}\label{eq:bound2-new}
  %I[X_{-M\dots 0}, X_{0\dots M} | Z] \leq -	\mathbb{E}_{z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M} | z)\right]- H[\finitefuture|X_{-M\dots-1}]
  \operatorname{H}[\finitefuture|Z] \leq   - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(\finitefuture | Z)\right]. % \operatorname{H}[\finitefuture]
\end{equation}
...
Let $Z$ be $S'_0$ if $S_0$ is identifiable and some uniformly random state otherwise.
	Then
	\begin{equation}
		\operatorname{I}[S'_0, \past] - \epsilon \operatorname{H}[S'_0]\leq	\operatorname{I}[Z, X_{-M, ..., 0}] \leq \operatorname{I}[S'_0, \past]
	\end{equation}
	\begin{equation}
		\operatorname{I}[S'_0,\future] \geq 	\operatorname{I}[Z, X_{1...M}] \geq \operatorname{I}[S'_0, X_{1...M}] - \epsilon \operatorname{H}[S'_0] \geq \operatorname{I}[S'_0, \future] - \operatorname{I}[S'_0, X_{M+1...\infty}|X_{1...M}] - \epsilon \operatorname{H}[S'_0] \geq \operatorname{I}[S'_0, \future] _ 2\epsilon \operatorname{H}[S'_0]
	\end{equation}
\end{proof}
...
$\operatorname{H}[M_0] = \operatorname{H}[M_0 | \future] + \operatorname{I}[M_0, \future]$
The first part is Ambiguity cost.
Now we show that the second part is equal to Locality cost.
As we assume that the listeners predictions are optimal, $\operatorname{I}[M_0, \future] = \operatorname{I}[X_{\leq 0}, \future]$.
...
%& \geq  - \frac{1}{T} \E_{X} \log P(X_{1\dots T} | f(\past)) - \log P(X_{1\dots T} | \past)  \\
...
\begin{equation}\label{eq:ib}
	\max_{Z} \left( \operatorname{I}[\future, Z] - \beta \cdot \operatorname{I}[Z, (X_{<0})] \right)
\end{equation}
where the scope of the maximization is the class of all random variables $Z$ such that $Z - \left(X_{<0}\right) - \left(\future\right)$ is a Markov chain, that is, $Z$ is independent of the future given the past.
