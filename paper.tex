...
%	P(X_{0\dots M-1}|Z) = \frac{\E_{X_{-M \dots M-1}} P_\phi(Z|X_{-M\dots-1})}{\E_{X_{-M \dots -1}} P_\phi(Z|X_{-M \dots -1})}
%\end{equation}
%
\begin{equation}
	\label{eq:cond-fut}
...
%We use held-out data to compute empirical estimates of quantities anlogous to those computed by NPRD.
...
\end{equation}
%\E_{z \sim q(\cdot|x_i)} \log \frac{q(z|x_i)}{s(z)} $$
...
%\rljf{More detail on this part: it's a bit telegraphic}% even though $Z$ is a multivariate Gaussian variable. 
