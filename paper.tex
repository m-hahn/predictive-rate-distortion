...
We conjecture that, due to the convolutional nature of QRNNs, they cannot model such processes effectively. \rljf{We'll want a bit more detail on this: someone might think we just made some mistake in applying the QRNNs.}
...
%\includegraphics[width=0.45\textwidth]{code/figures/even-beta-mem.pdf}
	\caption{Rate-Distortion for the Even Process (left) and the Random Insertion Process (right), varying $M=5, 10, 15$. Gray lines: Analytical curves. Red dots: Multiple runs of NPRD. Red line: Tradeoff curve computed from NPRD runs. Compare Figure~\ref{fig:even} for results from full NPRD. \mhahn{TODO fix the figures}}\label{fig:even-varyM}
\end{figure*}
