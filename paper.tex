...
Fix $\epsilon$ and $M \in \mathbb{N}$.
	...
	Let $(X_t)_t \sim P$, $(Y_t)_t \sim Q$ be two $M$-th order Markov processes. Assume that
	$$\E_{X_{-\infty}...X_0} \left[\log \frac{P(X_1|X_{-\infty}...X_0)}{Q(X_1|X_{-\infty}...X_0)}\right] < \epsilon$$
	
	For each point $(R,P$) on the PRD of $(Y_t)_t$, the point $(R+M\epsilon, P - M \epsilon)$ is an upper bound on the PRD of $(X_t)_t$.
...
\begin{corollary}
If $P, P_n$ are the distributions of $M$-th order Markov processes ($M$ fixed), and if
