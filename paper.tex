...
Note that the predictive rate-distortion objective~(\ref{eq:ib-oce-repeat}) can equivalently be written as
...
\begin{equation}\label{eq:eq2-new}
  ...
  \operatorname{H}[X_{0\dots M-1}|Z] =   - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] %H[X_{0\dots M-1}|X_{-M\dots-1}] % \operatorname{H}[X_{0\dots M-1}]
\end{equation}
...
If $\phi, \psi, q$ are not exact solutions to (\ref{eq:bound}), the two propositions guarantee that we still have bounds on rate and prediction error for the code $Z$ generated by $\phi$:
...
\begin{equation}\label{eq:bound2-new}
  ...
  \operatorname{H}[X_{0\dots M-1}|Z] \leq   - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] % \operatorname{H}[X_{0\dots M-1}]
\end{equation}
...
We then use the held-out test set to estimate rate and prediction error.
...
We estimate the prediction error on the future observations as the empirical cross-entropy, i.e., the variational bound stated in Proposition~\ref{ineq1}:
\begin{equation}
