...
\begin{equation}\label{eq:ib-oce-repeat}
	\max_{Z : Z \bot X_{0\dots M-1} | X_{-M\dots-1}} \left( \operatorname{I}[Z, X_{0\dots M-1}] - \lambda \cdot \operatorname{I}[X_{-M\dots -1}, Z] \right)
\end{equation}
...
%\end{equation}
It will be convenient to equivalently rewrite this as
\begin{equation}\label{eq:prd-loss}
	\min_{Z : Z \bot X_{0\dots M-1} | X_{-M\dots-1}}	\left[\operatorname{H}[X_{0\dots M-1} | Z] + \lambda \cdot\operatorname{I}[ X_{-M\dots-1}, Z]\right]
\end{equation}
where $\operatorname{H}[X_{0\dots M-1} | Z]$ is the \emph{prediction loss}. Note that minimizing prediction loss is equivalent to maximizing predictiveness $\operatorname{I}[X_{0\dots M-1} | Z]$. % is the reduction in prediction loss when predicting
...
Given that NPRD \emph{provably} provides \emph{upper bounds} on rate and prediction loss (up to sampling error), one can objectively compare the quality of different estimation methods:
...
Given the estimate for prediction loss, we estimate predictiveness $\operatorname{I}[Z, X_{0\dots M-1}]$ with the following method.
...
\paragraph{OCE}
As discussed in Section~\ref{sec:oce}, OCE is affected by overfitting, %overestimates structure in the process due to overfitting, %resulting in biased estimates of rate and distortion.More specifically, 
%overfitting will result in an overly optimistic estimate, 
...
\paragraph{Implication 1: Less information locality requires more memory at the same level of prediction loss}
