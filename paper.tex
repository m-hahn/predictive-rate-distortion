...
\title{Estimating Predictive Rate-Distortion Curves via Neural Variational Inference}
\date{2018}
...
\begin{abstract}
The predictive rate-distortion curve quantifies the tradeoff between compressing information about the past of a stochastic process and predicting its future accurately.
...
Based on the results, we argue that the predictive rate-distortion curve is a useful complexity metric for highly complex processes such as natural language.
\end{abstract}
...
%Prediction has been argued to permeat the behavior of organisms at all levels:
%\textbf{TODO some more about biological systems, language, ...?}
...
They also have been successfully applied to modeling many other kinds of time series found across discipline~\citep{ogunmolu2016nonlinear,laptev2017time,meyer2018survey,hundman2018detecting,white2018forecasting,woo2018dynamic,sirignano2018universal,mohajerin2019multistep}. %TODO more details
...
We consider stationary stochastic processes $(X_t)_{t \in \mathbb{Z}}$, taking values in a state space $S$.
...
We will write $\overleftarrow{X}$ for the past $X_{< 0}$, and $\overrightarrow{X}$ for the future $X_{\geq 0}$, following \cite{marzen-predictive-2016}.
We consider random variables $Z$, called \emph{codes}, that summarize the past and are used by the observer to predict the future.
...
\end{equation}
This is equivalent to the requirement that $Z \rightarrow \overleftarrow{X} \rightarrow\overrightarrow{X}$ be a Markov chain.
This formalizes the idea that the code is computed by an observer from the past, without having access to the future.
Predictions are then made based only on $Z$, without additional access to the past $\overleftarrow{X}$.
...
\begin{equation}
	\operatorname{I}[\overleftarrow{X}, \overrightarrow{X}|Z] = \operatorname{I}[\overleftarrow{X}, \overrightarrow{X}] - \operatorname{I}[Z, \overrightarrow{X}]
\end{equation}
i.e., it captures how much less information $Z$ carries about the future $\overrightarrow{X}$, compared to the full past $\overleftarrow{X}$.
...
\operatorname{I}[Z, \overrightarrow{X}]
\end{equation}
We will refer to~(\ref{eq:predictiveness}) as the \emph{predictiveness} of the code $Z$.

The rate-distortion tradeoff then chooses $Z$ to %minimize rate at bounded distortion:
%$$\min_{Z : \operatorname{I}[\overleftarrow{X}, \overrightarrow{X}|Z] \leq d} \operatorname{I}[Z, \overleftarrow{X}]$$
%, or -- equivalently --
 minimize distortion at bounded rate:
\begin{equation}
\min_{Z :  \operatorname{I}[\overleftarrow{X}, Z] \leq d}  \operatorname{I}[\overleftarrow{X}, \overrightarrow{X}|Z] 
\end{equation}
or -- equivalently -- maximize predictiveness at bounded rate:
\begin{equation}
\max_{Z :  \operatorname{I}[\overleftarrow{X}, Z] \leq d}  \operatorname{I}[Z, \overrightarrow{X}] 
\end{equation}
Equivalently, for each  $\lambda \geq 0$, we study the problem 
\begin{equation}\label{eq:ib}
	\max_{Z} \left( \operatorname{I}[Z, \overrightarrow{X}] - \lambda \cdot \operatorname{I}[ \overleftarrow{X}, Z] \right)
\end{equation}
where the scope of the maximization is the class of all random variables $Z$ such that $Z \rightarrow \overleftarrow{X} \rightarrow \overrightarrow{X}$ is a Markov chain. %, that is, $Z$ is independent of the future given the past.
...
The coefficient $\lambda$ indicates how strongly a high rate $\operatorname{I}[\overleftarrow{X}, Z]$ is penalized; higher values of $\lambda$ result in lower rates and thus lower values of predictiveness.


The largest achievable predictiveness $\operatorname{I}[Z, \overrightarrow{X}]$ is equal to $\operatorname{I}[\overleftarrow{X}, \overrightarrow{X}]$, which is known as the \emph{excess entropy} of the process~\citep{feldman-synchronizing-2004}.
...
\begin{equation}
\operatorname{I}[Z, \overrightarrow{X}] \leq \operatorname{I}[\overleftarrow{X}, Z] 
\end{equation}
...
%
%The complexity of the representation $Z$, or the \emph{rate}, is formalized as the mutual information between the past and the representation $Z$: $\operatorname{I}[Z, \overleftarrow{X}]$.
%Different distortion measures could be used as measures of prediction error.
%CITE maximize the entropy, or, equivalently, minimize the mutual information, between $Z$ and the future: $\operatorname{I}[Z, \overleftarrow{X}]$.
%
...
%\begin{equation}\label{eq:ib}
%	\max_{Z} \left( \operatorname{I}[\overrightarrow{X}, Z] - \beta \cdot \operatorname{I}[Z, (\overleftarrow{X})] \right)
%\end{equation}
...
Two pasts $\overleftarrow{x}, \overleftarrow{x}'$ belong to the same causal state if and only if $P(x_{1...M}|\overleftarrow{x}) = P(x_{1...M}|\overleftarrow{x}')$ holds for all finite sequences $x_{0...M}$ ($M \in \mathbb{N}$).
...
With such an estimator, the problem in (\ref{eq:ib}) can be solved using a variant of the Blahut-Arimoto algorithm \citep{tishby-information-1999}, obtaining an encoder $P(Z|X_{-M...-1})$ that maps each observed past sequence $X_{-M...-1}$ to a distribution over a (finite) set of code words $Z$.
%From this, one obtains a corresponding decoder $P(X_{0...M}|Z)$ via Bayes' Rule.
...
Given that storing and manipulating matrices greater than $10^5 \times 10^5$ is currently not feasible, a setting where $\operatorname{I}[\overleftarrow{X}, Z] > log(10^5) \approx 11.5$ cannot be captured with OCE.
...
As in OCE (Section~\ref{sec:oce}), we assume that a set of finite sample trajectories $x_{-M} \dots x_{M-1}$ is available, and we aim to compress pasts of length $M$ to predict futures of length $M$. To carry this out, we restrict the PRD objective~(\ref{eq:ib}) to such finite-length pasts and futures:
\begin{equation}\label{eq:ib-oce-repeat}
	\max_{Z} \left( \operatorname{I}[X_{-M\dots -1}, Z] - \lambda \cdot \operatorname{I}[Z, X_{0\dots M-1}] \right)
\end{equation}
where $Z$ ranges over all random variables such that the Markov condition~(\ref{eq:markov}) holds:
\begin{equation}
    Z\bot X_{0\dots M-1} | X_{-M\dots -1}
\end{equation}

When deploying such a predictive code $Z$, two components have to be computed: a distribution $P(Z|X_{-M\dots -1})$ that \emph{encodes} past observations into a code $Z$, and a distribution $P(X_{0\dots M-1}|Z)$ that \emph{decodes} the code $Z$ into predictions about the future.

Let us assume that we already have some encoding distribution
\begin{equation}
Z \sim P_\phi(Z|X_{-M\dots -1})
\end{equation}
defined using an \emph{encoder} $\phi$, which is expressed in some family of function approximators that transduces an observation sequence $X_{-M\dots -1}$ into the parameters of the distribution $P_\phi(\cdot|X_{-M\dots -1})$.
...
\end{equation}
Second, to approximate the decoding distribution $P(X_{0\dots M-1}|Z)$, we introduce a parameterized decoder $\psi$ that maps points $Z \in \mathbb{R}^N$  into probability distributions $P_\psi(X_{0\dots M-1}|Z)$ over future observations $X_{0\dots M-1}$:
\begin{equation}
P_\psi(X_{0\dots M-1}|Z)  \text{ approximates }  P(X_{0\dots M-1}|Z)
\end{equation}
for each code $Z \in \mathbb{R}^N$. Crucially, $P_\psi(X_{0\dots M-1}|Z)$ will be easy to compute efficiently, unlike the exact decoding distribution $P(X_{0\dots M-1}|Z)$.
...
\begin{equation}\label{ineq1}
	-	\mathbb{E}_{X}\mathbb{E}_{Z \sim \phi(X)}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] \geq \operatorname{H}[X_{0\dots M-1}|Z]
\end{equation}
Furthermore, equality is attained if and only if $P_\psi(X_{0\dots M-1}|Z) = P(X_{0\dots M-1}|Z)$. % (as defined in~(\ref{eq:cond-fut})).
\end{proposition}
...
\begin{align*}
	...
	&= \operatorname{H}[X_{0\dots M-1}|Z].
\end{align*}
\end{proof}

\begin{proposition} 
The KL Divergence between $P_\phi(Z|X_{-M\dots-1})$ and $q(Z)$, averaged over pasts $X_{-M\dots-1}$, upper-bounds the rate of $Z$: % mutual information between $Z$ and the past observations:
...
We now use the two propositions to rewrite the predictive rate-distortion objective~(\ref{eq:ib-oce-repeat}) in a way amenable to using function approximators.
Note that the the predictive rate-distortion objective~(\ref{eq:ib-oce-repeat}) can equivalently be written as
\begin{equation}\label{eq:prd-recall}
	\min_{Z : Z \bot X_{0\dots M-1} | X_{-M\dots-1}}	\left[\operatorname{H}[X_{0\dots M-1} | Z] + \lambda \operatorname{I}[ X_{-M\dots-1}, Z]\right]
\end{equation} % should be P_\psi (X_{0...M}|z)? % done (mhahn)
By the two Propositions, this objective is equivalent to
\begin{equation}\label{eq:bound}
	\min_{\phi, \psi, q}	\left[-	\mathbb{E}_{Z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] + \lambda \cdot \operatorname{D_{KL}}\infdivx{P_\phi(\cdot|X_{-M\dots-1})}{q}\right],
\end{equation} % should be P_\psi (X_{0...M}|z)? % done (mhahn)
where $\phi, \psi, q$ range over all triples of the appropriate types described above.
From a solution to (\ref{eq:bound}), one obtains a solution to (\ref{eq:prd-recall}) by setting $Z$ to be the code generated by the encoder $\phi$.
The second proposition guarantees that, for a solution to  (\ref{eq:prd-recall}), the rate of this code is given as follows
...
\end{equation}
and the predictiveness is given by
\begin{equation}\label{eq:eq2-new}
  %H[X_{0\dots M} | Z] =  -	\mathbb{E}_{z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M} | z)\right]
  \operatorname{I}[Z, X_{0\dots M-1}] =  \operatorname{H}[X_{0\dots M-1}] - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] %H[X_{0\dots M-1}|X_{-M\dots-1}]
\end{equation}
...
If $\phi, \psi, q$ are not exact solutions to (\ref{eq:bound}), the two propositions guarantee that we still have bounds on rate and predictiveness for the code $Z$ generated by $\phi$:
...
\begin{equation}\label{eq:bound2-new}
  %I[X_{-M\dots 0}, X_{0\dots M} | Z] \leq -	\mathbb{E}_{z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M} | z)\right]- H[X_{0\dots M-1}|X_{-M\dots-1}]
  \operatorname{I}[Z, X_{0\dots M-1}] \geq  \operatorname{H}[X_{0\dots M-1}] - \mathbb{E}_{X_{-M\dots M-1}}	\mathbb{E}_{Z \sim P_\phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right]
\end{equation}
...
\paragraph{Approximating Families}
We choose the approximating families for the encoder $\phi$, the decoder $\psi$, and the distribution $q$ to be certain types of neural networks.

For $\phi$ and $\psi$, we use recurrent neural networks with LSTM cells \citep{hochreiter-long-1997}, widely used for modeling sequential data across different domains.
...
For the decoder $\psi$, we use a second LSTM network to compute a sequence of vector representations $g_t = \psi(Z, X_{1\dots t-1})$ ($g_t \in \mathbb{R}^k$) for $t = 1, \dots T$.
...
Following standard practice in machine learning, we split the data set of sample time series into three partitions (\emph{training set}, \emph{validation set}, and \emph{test set}).
...
After optimizing the parameters on a set of observed trajectories, we estimate rate and prediction error on the test set.
Given parameters for $\phi, \psi, q$, we evaluate the PRD objective~(\ref{eq:bound}), rate (\ref{eq:bound1-new}), and the prediction error (\ref{eq:bound2-new}) on the test set by taking, for each time series $X_{-M}...X_{M-1}$ from the test set, a single sample $z  \sim \mathcal{N}(\mu, \sigma^2)$ and computing Monte Carlo estimates for rate
...
\end{equation}
where $p_{\mathcal{N}(\mu, \sigma^2)}(z)$ is the Gaussian density with $\mu, \sigma^2$ computed from $X_{-M\dots -1}$ as in (\ref{eq:z-gauss}), and prediction error 
\begin{equation}\label{eq:bound-mc-distortion}
 -	\mathbb{E}_{Z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M-1} | Z)\right] \approx \frac{1}{N}	\sum_{X_{-M...M} \in TestData}	\log P_\psi(X_{0\dots M-1} | Z)
\end{equation}
...
Thanks to (\ref{eq:bound1-new}-\ref{eq:bound2-new}), these estimates are guaranteed to be \emph{upper bounds} on the true rate and prediction error achieved by the code $Z  \sim \mathcal{N}(\mu, \sigma^2)$, up to sampling error introduced into the Monte Carlo estimation by sampling $z$ and the finite size of the test set.
...
(\ref{eq:bound-mc-rate}-\ref{eq:bound-mc-distortion}) provide \emph{unbiased} estimators upper bounds, whereas overfitting \emph{biases} the values obtained by OCE.
Given that NPRD \emph{provably} provides \emph{upper bounds} on rate and prediction error (up to sampling error), one can objectively compare the quality of different estimation methods:
...
We estimate predictiveness $\operatorname{I}[Z, X_{0\dots M-1}]$ with the following method.
%by estimating $\operatorname{H}[X_{0\dots M-1}]$ 
Together with $\psi$, we also create an LSTM network that estimates the marginal probability of future observation sequences $P(X_{0\dots M-1})$. Let $P_\eta$ this estimated distribution; we then estimate the predictiveness $\operatorname{I}[Z, X_{0\dots M-1}] = \operatorname{H}[X_{0\dots M-1}] - \operatorname{H}[X_{0\dots M-1}|Z]$ by the difference between the corresponding cross-entropies on the test set~(\cite{mcallester2018formal}):
%$\operatorname{H}[X_{0\dots M-1}]$ by the cross-entropy of $P_\eta$ on the test data:
\begin{equation}\label{eq:bound-mc-predictiveness}
- \mathbb{E}_{X_{-M\dotsM-1}}\left[\log P_\eta(X_{0\dots M-1}) - P_\psi(X_{0\dots M-1} | Z)\right]
\end{equation}
...
%\begin{equation}\label{eq:bound1-new}
% I[Z, X_{-M\dots-1}] \leq   \mathbb{E}_{X}\left[ \operatorname{D_{KL}}\infdivx{P(Z|X)}{q(Z)}\right]
%\end{equation}
%\begin{equation}\label{eq:bound2-new}
%  H[X_{0\dots M} | Z] \leq -	\mathbb{E}_{z \sim \phi(X_{-M\dots-1})}\left[\log P_\psi(X_{0\dots M} | z)\right]
%\end{equation}
...
This is formally equivalent to a variational formulation of the Information Bottleneck that was introduced by \cite{alemi-deep-2016}, who applied it to neural-network based image recognition.
Unlike our approach, they used a fixed diagonal Gaussian instead of a parametrized distribution for $q$.
As far as we are aware, no previous work has used such a formulation in the modeling of sequences.
...
We now test the ability of NPRD to estimate rate-distortion curves.
...
\paragraph{OCE}
As discussed in Section~\ref{sec:oce}, OCE overestimates structure in the process due to overfitting, %resulting in biased estimates of rate and distortion.More specifically, 
overfitting will result in an overly optimistic estimate, systematically overestimating the predictiveness achieved at a given rate.
To address this problem, we follow the evaluation method used for NPRD, evaluating rate and distortion on held-out test data.
...
In the limit of enough training data, when $s(Z)$ matches the actual population marginal of $Z$, (\ref{eq:rate-oce}) is an unbiased estimate of the rate. %  If the training set is large enough and 
...
\begin{equation}
\frac{1}{N} \sum_{X_{-M}\dots X_{M-1} \in \text{Test Data}} \E_{Z \sim P(\cdot|X_{-M \dots -1})} \log P(X_{0\dots M-1}|Z).
\end{equation}
where $P(X_{0\dots M-1}|Z)$ is the decoder obtained from the Blahut-Arimoto algorithm on the training set.
...
Larger dimensions, more layers, and longer flows lead to more expressive models; however, they are computationally expensive.
...
The asymptote of predictiveness can be identified with the excess entropy $E_0 := \operatorname{I}[X_{-M\dots -1}, X_{0 \dots M-1}]$, whereas the rate should asymptote to the statistical complexity.
...
\end{equation}
Our ansatz therefore leads to the differential equation
...
\includegraphics[width=0.3\textwidth]{code/figures/english-logbeta-mem-fitted.pdf}
	...
	\caption{Interpolated values for POS-level prediction of English (compare Figure~\ref{fig:eng-pos}) \textbf{[TODO sign of center figure]}}\label{fig:eng-pos-fitted}
\end{figure*}
...
Note that there are many possible ways of fitting these curves; we have described a simple one that requires two free parameters $\alpha >0$, $\beta > 1$, in addition to a guess for the maximal predictiveness $E_0$.
...
One natural idea is that the causal states of natural language are the singletons $\overleftarrow{X}$, that is, no two past trajectories induce a fully equivalent distribution over future trajectories.
...
%\end{equation}
%which is lower bounded by $T$ times the entropy rate: $R \geq T \cdot \operatorname{H}[X_0|\overleftarrow{X}]$, and thus is unbounded, assuming that the entropy rate is nonzero.
...
%We verified that NPRD matches OCE in the low-rate regime where OCE is still applicable, and we fitted 
...
This problem is much closer to the task faced by a human in the process of comprehending text, having to encode prior observations so as to minimize prediction error on the upcoming words.
...
At the time scale of individual sentences, predictive rate-distortion of natural language is quantitatively described by Equation~\ref{eq:derivatives}.
...
\begin{align*}
\operatorname{I}[X_{<0}, X_{>0}] &= \sum_{t=1}^\infty t \operatorname{I}[X_t, X_0|X_1\dots X_{t-1}] \\
...
Thus
$$\operatorname{H}[X_1...X_{t_i} | X \in \mathcal{X}] \geq  t_i \log \frac{\mu(0)}{2} = \Theta(t_i)$$
In conclusion, the statistical complexity is infinite.


%For any positive integer $T$ and any string $x_1...x_T \in \{0,1\}^T$, we have $\log P(X_1...X_T=x_1...x_T) \geq T \log\frac{\mu(\{0\})}{2}$, showing that $$\operatorname{H}[X_1...X_T] \geq T \log\frac{\mu(\{0\})}{2} \rightarrow_{T \rightarrow\infty}\infty$$
...
Its distortion is $\operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}]$.
...
\begin{align*}
\operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] & = \sum_{t=T}^\infty (t-T) \operatorname{I}[X_t, X_0|X_1\dots X_{t-1}] \\
...
\end{align*}}
and a lower bound can similarly be derived. \footnote{$\operatorname{I}[X_t, X_0|X_1\dots X_{t-1}]$ is at least the probability that the block did not copy from $X_0$, but $X_t$ does.
...
\begin{equation}
\E \left[(t-T)_+ \right] \left( \prod_{j=1}^\infty (1-\mu(j))\right) \leq \operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] \leq 1/2 \cdot \E[(k-T)_+^2-(k-T)_+]
\end{equation}
...
\begin{equation}
T \frac{q^{T+1}}{1-q}  \phi(q) (1-q) \leq \operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] \leq 1/2 \cdot (\frac{q+q^2}{(1-q)^2} T q^T (1-q) - T \frac{q^{T+1}}{1-q} )
\end{equation}
and thus
\begin{equation}
\operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] = \Theta(T q^T)
\end{equation}
...
\begin{equation}
\Theta(T^{-t+2} + T^{-t+1}) \leq \operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] \leq \Theta(T^{-t+3} + T^{-t+2})
\end{equation}
and thus
\begin{equation}
\operatorname{I}[X_{\leq 0}, X_{>T}|X_{1\dots T}] = \Theta(T^{-t+2})
\end{equation}
...
\begin{proposition}
Let $(X_t)_t$ be a process such that the entropy $\operatorname{H}[X_t]$ is finite (e.g., if the state space is finite).
...
\begin{proof}
An upper bound on the curve is given by the (convex hull) of the pairs of $$\{(\operatorname{H}[X_1...X_t], \operatorname{I}[(X_1...X_t), X_{>t}]) : t \in \mathbb{N}\}$$
all which are finite under the assumption. 
Also, $$EE = \sup_{t \in \mathbb{N}} \operatorname{I}[(X_1...X_t), X_{>t}]$$
...
%%	$$\E_{X_{-\infty}...X_0} \left[\log \frac{P(X_1|X_{-\infty}...X_0)}{Q(X_1|X_{-\infty}...X_0)}\right] < \epsilon$$
	%$D_{KL}((X_{1...M}) || Y_{1...M} | \overleftarrow{X}) < \epsilon \operatorname{H}[S_0]$.
...
\end{align*}
We always know that $- \left(\sum_z p(z|x_{-M...-1}) \log \frac{p(z|x_{-M...-1})}{p_Y(z)}\right) \leq \frac{\operatorname{I}[Z,X]}{\exp(-K)} \leq \log |S| \cdot M \cdot \exp(K)$.
Therefore
\begin{align*}
...
%Storing $S'_0$ and predicting the future on its basis results in $\epsilon \operatorname{H}[S_0]$ additional loss in the future.
\end{proof}
...
%	Further, assume $\operatorname{I}[X_{>M}, X_{<0}|X_{1...M}] < \epsilon_2$.
%	$$\E_{X_{-\infty}...X_0} \left[\log \frac{P(X_1|X_{-\infty}...X_0)}{Q(X_1|X_{-\infty}...X_0)}\right] < \epsilon$$
	%$D_{KL}((X_{1...M}) || Y_{1...M} | \overleftarrow{X}) < \epsilon \operatorname{H}[S_0]$.
...
%
%
%$K_M := \operatorname{H}[S_0|X_{-M\dots 0}]$ is an upper bound on the statistical complexity
%
%$K_M := \operatorname{I}[S_0, X_{-M\dots 0}]$ is a lower bound on the statistical complexity
%
%the statistical complexity of the $2M$-order Markov approximation is a lower bound on the statistical complexity
%
%If $\operatorname{I}[S_0, X_{\leq 0}] - K_M \epsilon$, then predictive rate-distortion estimated with trajectories of length $M$ will be close to tight.
%
...
%%$$\inf_{Z \rightarrow X_{-M...0} \rightarrow X_{1...M} : \operatorname{I}[Z, X_{1...M}] = \operatorname{H}[Z]} \operatorname{H}[Z]$$
%%$$\inf_{Z \rightarrow X_{-\infty...0} \rightarrow X_{1...M} : \operatorname{I}[Z, X_{1...M}] = \operatorname{H}[Z]} \operatorname{H}[Z]$$
%
%or 
%$K_M := \operatorname{I}[S_0, X_{-M\dots 0}]$
%
%
%
%
...
%\section{Discussion}
%
%In (\ref{eq:bound}), we upper-bounded $\operatorname{I}[Z, (X_{-T\dots -1})]$ using $D_{KL}(N(\mu, \sigma^2) || q)$.
%There are other ways of approximating $\operatorname{I}[Z, (X_{-T\dots -1})]$ that (for fixed $Z$) give a lower bound instead of an upper bound.
...
\item	$\frac{\operatorname{I}[S_0, X_{M+1...\infty}|X_{1...M}]}{\operatorname{H}[S_0]} < \epsilon$
	\end{enumerate}
%Assume that the epsilon machine has $\epsilon/2$-mixing time $< M$ (in the sense that $\operatorname{I}[S_0, S_M] < \epsilon/2$), 
	Then the predictive rate-distortion curve of the $2M$-order Markov approximation is $3\epsilon \operatorname{H}[S_0]$-close to the curve of $(X_t)_t$.
	%Assume that $\operatorname{H}[S_0|X_{-M\dots 1}] < \epsilon/2$ and that
\end{proposition}
...
Let $Z$ be $S'_0$ if $S_0$ is identifiable and some uniformly random state otherwise.
	Then
	\begin{equation}
		\operatorname{I}[S'_0, \overleftarrow{X}] - \epsilon \operatorname{H}[S'_0]\leq	\operatorname{I}[Z, X_{-M, ..., 0}] \leq \operatorname{I}[S'_0, \overleftarrow{X}]
	\end{equation}
	\begin{equation}
		\operatorname{I}[S'_0,\overrightarrow{X}] \geq 	\operatorname{I}[Z, X_{1...M}] \geq \operatorname{I}[S'_0, X_{1...M}] - \epsilon \operatorname{H}[S'_0] \geq \operatorname{I}[S'_0, \overrightarrow{X}] - \operatorname{I}[S'_0, X_{M+1...\infty}|X_{1...M}] - \epsilon \operatorname{H}[S'_0] \geq \operatorname{I}[S'_0, \overrightarrow{X}] _ 2\epsilon \operatorname{H}[S'_0]
	\end{equation}
\end{proof}
...
%	$$\E_{X_{-\infty}...X_M} \left[\log \frac{P(X_1...X_M|X_{-\infty}...X_0)}{Q(X_1...X_M|X_{-\infty}...X_0)}\right] < \epsilon \operatorname{H}[S_0]$$
%%	$D_{KL}((X_{1...M}) || Y_{1...M} | \overleftarrow{X}) < \epsilon \operatorname{H}[S_0]$.
%	Then the predictive rate-distortion curve of the $2M$-order Markov approximation of $(Y_t)_t$ is $5\epsilon \operatorname{H}[S_0]$-close to the curve of $(X_t)_t$.
%
%\end{proposition}
%
%\begin{proof}
...
A := \operatorname{H}[M_0|\overrightarrow{X}]
\end{equation}
...
$\operatorname{H}[M_0] = \operatorname{H}[M_0 | \overrightarrow{X}] + \operatorname{I}[M_0, \overrightarrow{X}]$
The first part is Ambiguity cost.
Now we show that the second part is equal to Locality cost.
As we assume that the listeners predictions are optimal, $\operatorname{I}[M_0, \overrightarrow{X}] = \operatorname{I}[X_{\leq 0}, \overrightarrow{X}]$.
Then the formula is a consequence of the chain rule for mutual information:
\begin{align*}
\operatorname{I}[(X_i)_{i > 0}, (X_i)_{i \leq 0}] &= \sum_{i=1}^\infty I(X_i, (X_i)_{i \leq 0}|X_1, ..., X_{i-1}) \\
...
The number of bits of memory is lower bounded by the mutual information between past and future observations, $\operatorname{I}[(X_i)_{i>0}, (X_i)_{i \leq 0}]$. 
Then the formula is a consequence of the chain rule for mutual information:
\begin{align*}
\operatorname{I}[(X_i)_{i > 0}, (X_i)_{i \leq 0}] &= \sum_{i=1}^\infty I(X_i, (X_i)_{i \leq 0}|X_1, ..., X_{i-1}) \\
&= \sum_{i=1}^\infty \sum_{j=0}^{-\infty} I(X_i, X_j|X_{j+1}...X_{i-1}) \\
...
Information locality is maximised if all contextual information predicting $X_t$ is contained in $X_{t-1}$, in which case $J = \operatorname{I}[X_t, X_{t-1}]$.

To quantify information locality more generally, we consider the relation between the quantity
$$J = \operatorname{I}[X_{\geq t}, X_{<t}] = \sum_{t=1}^\infty\ t \cdot \operatorname{I}[X_t, X_0 | X_1, ..., X_{t-1}]$$
and the quantity
$$P = \operatorname{I}[X_t, X_{< t}] = \sum_{t=1}^\infty\ \operatorname{I}[X_t, X_0 | X_1, ..., X_{t-1}]$$
...
Prior work has studied the decay of \emph{unconditional} mutual information $\operatorname{I}[X_t, X_0]$ in natural language, and linked it to locality and memory.
...
Note that the quantities $\operatorname{I}[X_t, X_0 | X_1, ..., X_{t-1}]$ in~(\ref{eq:memory-bound}) are equal to difference between conditional entropies $\operatorname{H}[X_t|X_1, ..., X_{t-1}] - \operatorname{H}[X_t|X_0, X_1, ..., X_{t-1}]$.
...
$$\E{f,p} \log(\pi(p)|f) = \operatorname{H}[\pi(P) | F]$$

$\operatorname{H}[\pi(P)] = \operatorname{I}[P,F] + \operatorname{H}[\pi(P) | F] = \operatorname{I}[\pi(P), F] + \operatorname{H}[\pi(p) | F]$
...
- variational: stochastic continuous memory representations and take memory cost to be $\operatorname{I}[Memory, Past]$ (also done in IB)

(noise the LSTM hidden state in each time step)

calculate upper bound $D_{KL}(P(Memory|Past) || P_{prior}(Memory)) \geq \operatorname{I}[Memory, Past]$, estimate the tradeoff curve between this and cross-entropy (the resulting curve is a variational upper bound on the real curve).
...
The average surprisal experienced by such a model is $\operatorname{H}[X_t | f(X_{<t})]$, and is lower bounded by the true surprisal, $\operatorname{H}[X_t|X_{<t}]$.

This section provides some results relating the model's memory cost $\operatorname{H}[f(X_{<t})]$ to the attained surprisal $\operatorname{H}[X_t | f(X_{<t})]$.
...
The Bayes-optimal $n$-th order Markov model will experience average surprisal (that is, cross-entropy) exactly $$\operatorname{H}[X_t|X_{<t}] + \sum_{t > n} \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]$$
\end{proposition}
...
We write $$I_t := \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]$$
...
%For any $T \in \mathbb{N}$, a model with memory $J$ will experience average surprisal (that is, cross-entropy) at least $$\operatorname{H}[X_t|X_{<t}] + \sum_{t \leq T} \frac{t}{T} I_t + \sum_{t > T} I_t - J/T$$
%Let $\epsilon > 0$, $T \in \mathbb{N}$ such that $\sum_{t > T} t I_t < \epsilon$.
%$$\sum_{t > T} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}] < \epsilon$$
%Then a model with memory $J$ will experience average surprisal (that is, cross-entropy) at least $$\operatorname{H}[X_t|X_{<t}] + \frac{M-J-\epsilon}{T} + \sum_{t > T} I_t$$
%Choose any $T \in \mathbb{N}$.
Any model with memory $J$ will experience average surprisal (that is, cross-entropy) at least
$$\operatorname{H}[X_t|X_{<t}] +\sup_{T\in \mathbb{N}}\ \left[ \frac{\left(\sum_{t=1}^T t I_t\right)-J}{T} + \sum_{t > T} I_t\right]$$
%$$\operatorname{H}[X_t|X_{<t}] + \sup_{T\in \mathbb{N}}\ \left[\frac{M-J}{T} - \sum_{t > T} (t/T - 1) I_t\right]$$
\end{proposition}
%The second term confirms the intuition that the model's surprisal should grow with the difference $M-J$, but the strength of the bound depends on the choice of $T$.\footnote{The bound can be vacuous for small choices of $T$: If we choose $T = 1$, the bound simply evaluates to the trivial lower bound $\operatorname{H}[X_t] - J$, which may be even weaker than the likewise trivial lower bound $\operatorname{H}[X_t|X_{<t}]$. It will become clear below that the bound is always nonvacuous when $T$ is large enough.}
...
\begin{proof}
Let $f$ be the function mapping histories $X_{<t}$ to memory representations of the model. By assumption, $f(X_{<t})$ can be encoded with up to $J$ bits, so $\operatorname{H}[f(X_{<t})] \leq J$.
...
& \operatorname{H}[X_t | f(X_{<t})] - \operatorname{H}[X_t | X_{<t}] =  \frac{1}{T} \sum_{t=1}^{T} \left(\operatorname{H}[X_t | f(X_{<t})] - \operatorname{H}[X_t | X_{<t}]\right) \\
& \geq   \frac{1}{T} \left(\operatorname{H}[X_{1\dots T} | f(X_{\leq 0})] - \operatorname{H}[X_{1\dots T} | X_{\leq 0}]\right)  = \frac{1}{T} \left(\operatorname{I}[X_{1\dots T}|X_{\leq 0}] - \operatorname{I}[X_{1\dots T}|f(X_{\leq 0})]\right) \\
& = \frac{1}{T} \left(\sum_{t \leq T} t I_t + T \sum_{t > T} I_t - \operatorname{I}[X_{1\dots T}|f(X_{\leq 0})]\right) = \frac{1}{T} \left(M - \sum_{t > T} t I_t + T \sum_{t > T} I_t - \operatorname{I}[X_{1\dots T}|f(X_{\leq 0})]\right) \\
& \geq \frac{1}{T} \left(M + \sum_{t > T} (T-t) I_t - J\right) 
\end{align*}
\end{proof}
%We can restate this bound in the following two, maybe more insightful, ways:\footnote{Note that (1) and (2) make clear that the bound is guaranteed to be larger than the basic bound $\operatorname{H}[X_t|X_{<t}]$ when $T$ is chosen sufficiently large, but the gain over that trivial bound diminishes as $T \rightarrow \infty$. Finding an optimal $T$ that makes the bound as large as possible will depend on the process in question.}
%
%\begin{corollary}
%(1) A model with memory $J$ will experience average surprisal at least $$\operatorname{H}[X_t|X_{<t}] +\sup_{T\in \mathbb{N}}\ \left[ \frac{\left(\sum_{t=1}^T t I_t\right)-J}{T} + \sum_{t > T} I_t\right]$$
%
...
\begin{corollary}
Let $\epsilon > 0$. Then it takes at least (and possibly more than) $$\sup_{T \in \mathbb{N}} \left(\sum_{t=1}^T t I_t\right) + T\left(\sum_{t > T} I_t - \epsilon\right)$$ bits of memory to achieve surprisal $\leq \operatorname{H}[X_t|X_{<t}] + \epsilon$.
\end{corollary}
...
Then any model with memory $J$ will experience average surprisal at least $$\operatorname{H}[X_t|X_{<t}] + \sum_{t > T} I_t$$
...
\begin{corollary}
Let $\epsilon > 0$, then let $T$ be the largest $T$ such that $\sum_{t > T} I_t > \epsilon$. Then it takes at least (and possibly more than) $\left(\sum_{t=1}^T t I_t\right)$ bits of memory to achieve surprisal $\leq \operatorname{H}[X_t|X_{<t}] + \epsilon$.
\end{corollary}
...
Then we can lower-bound the surprisal of a model with memory $J$ by $\operatorname{H}[X_t|X_{<t}]$ plus (TODO algebra errors may lie ahead)
...
$$ \operatorname{H}[X_t|X_{<t}] + \frac{[\frac{\alpha}{\beta-2} - J]^{\frac{\beta-1}{\beta-2}}}{\left((1-\beta) [ \frac{\alpha}{\beta-1} - \frac{\alpha}{\beta-2}]\right)^{1/(\beta-2)}} + \frac{[\frac{\alpha}{\beta-2} - J]^{\frac{\beta+1}{\beta-2}}}{(\beta-1)^{\frac{\beta+1}{\beta-2}} [ \frac{\alpha}{\beta-2} - \frac{\alpha}{\beta-1}]^{\frac{\beta+1}{\beta-2}-1}}$$
which grows monotonically, with slightly superlinear speed, as the approximating model's memory $J$ decreases.
Furthermore, if information locality in the process decreases -- that is, $\beta$ decreases but we adjust $\alpha$ to keep $\operatorname{I}[X_t|X_{<t}]$ constant -- it seems that the factor $(\beta-1)$ in both denominators will increase the bound, confirming the intuition that approximating a process with less information locality will require more memory.
\end{example}
...
%
%Any $n$-gram model will experience average surprisal (that is, cross-entropy) $$\operatorname{H}[X_t|X_{<t}] + \sum_{t > n} I_t$$
%\end{corollary}
%
%\begin{proof}
%
...
The Bayes-optimal $n$-th order Markov model achieves average surprisal $\leq \operatorname{H}[X_t|X_{<t}] + M/n$.
...
\begin{corollary}
(1) For any $\epsilon > 0$, there is a model achieving surprisal $\operatorname{H}[X_t|X_{<t}] + \epsilon$ using $\leq M \frac{\operatorname{H}[X]}{\epsilon}$ bits of memory.

(2) For any $J < M$, there is a model with memory $J$ that achieves average surprisal at most $$\operatorname{H}[X_t|X_{<t}] + \frac{M}{J} \operatorname{H}[X_t]$$

\end{corollary}
\begin{proof}
By the proposition, we can achieve a surprisal of $\operatorname{H}[X_t|X_{<t}] + \epsilon$ with an $M/\epsilon$-gram model.
This model can be implemented using 
$\operatorname{H}[X_1 ... X_{M/\epsilon}] \leq M \frac{\operatorname{H}[X]}{\epsilon}$ bits of memory.\footnote{
This bound is really crude. We can compute
\begin{align*}
\operatorname{H}[X_1 ... X_{M/\epsilon}] &= \frac{M}{\epsilon} \operatorname{H}[X] - \sum_{t=1}^{M/\epsilon} (M/\epsilon-t) I_t
%&= I/\epsilon \operatorname{H}[X] - I/\epsilon \left(\sum_{t=1}^{I/\epsilon} I_t\right) + \sum_{t=1}^{I/\epsilon} t I_t \\
%&\leq I/\epsilon \operatorname{H}[X] + I \\
%& = I(1+\operatorname{H}[X]/\epsilon) \\
\end{align*}
...
In the case of natural language, $\operatorname{H}[X]$ is a reasonably small constant.
...
%$$\operatorname{H}[X_t|X_{<t}] + \frac{M}{n} - \left(\sum_{t > n} (t/n - 1) I_t\right)\ \leq\ S_n\ \leq\ \operatorname{H}[X_t|X_{<t}] + \frac{M}{n}$$
%\end{corollary}
...
We quantify information (non-)locality by $\frac{\operatorname{I}[X_{\geq t}, X_{<t}]}{\operatorname{I}[X_t, X_{<t}]}$
...
%We consider the following question: For a fixed budget of predictability, $\operatorname{I}[X_t|X_{<t}]$, we want to minimize excess entropy -- that is, we want to 
...
$$\frac{\sum_{t=1}^\infty t \cdot \operatorname{I}[X_t,X_0 | X_{1...t-1}]}{\sum_{t=1}^\infty \operatorname{I}[X_t,X_0|X_{1...t-1}]}$$
...
\item This quantity stays invariant when we multiply each $\operatorname{I}[X_t,X_0 | X_{1...t-1}]$ with a fixed factor. This makes it a natural measure of the decay of $\operatorname{I}[X_t,X_0 | X_{1...t-1}]$ as $t \rightarrow \infty$.
...
& = - \frac{1}{T} \E_{X} \sum_{t=1}^T \log P(X_t | f(X_{<t})) - \log P(X_t | X_{<t}) \\
& \geq  - \frac{1}{T} \E_{X} \log P(X_{1\dots T} | f(\overleftarrow{X})) - \log P(X_{1\dots T} | \overleftarrow{X})  \\
& = \frac{1}{T} \left(\operatorname{H}[X_{1\dots T}|f(X_{<0})] -  \operatorname{H}[X_{1\dots T}|X_{<0}]\right) \\
& = \frac{1}{T} (\operatorname{I}[X_{1\dots T}|X_{<0}] - \operatorname{I}[X_{1\dots T}|f(X_{<0})]) \\
& = \frac{1}{T} (\sum_{t \leq T} t I_t + T \sum_{t > T} I_t - \operatorname{I}[X_{1\dots T}|f(X_{<0})]) \\
...
%If $\sum_{t > T} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}] < \epsilon$, then a model with memory $J$ will experience average surprisal (that is, cross-entropy) at least $\operatorname{H}[X_t|X_{<t}] + \frac{M-J-\epsilon}{T}$. TODO
%\end{proposition}
%\begin{proof}
%Let $f$ be the function mapping histories $X_{<t}$ to memory representations of the model. By assumption, $f(X_{<t})$ can be encoded with up to $J$ bits, so $\operatorname{H}[f(X_{<t})] \leq J$.
...
%%& \geq \frac{1}{T} (TODO - \operatorname{I}[X_{1\dots T}|f(X_{<0})]) \\
%%& \geq \frac{1}{T} (TODO M - J - \epsilon) \\
%\end{align*}
%\end{proof}

%If we consider ratios between the memory bound and the predictability $\operatorname{I}[X_t, X_{<t}]$, we can also express this as follows:
%\begin{corollary}
%Let $\epsilon > 0$, and let $P := \operatorname{I}[X_t, X_{<t}]$ the predictability.
%Then If $\sum_{t > T} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}] < \epsilon$, then a model with memory $J$ will experience average surprisal (that is, cross-entropy) at least $\operatorname{H}[X_t|X_{<t}] + \frac{M-J-\epsilon}{T}$.
%\end{corollary}
%
%\begin{proof}
%$\frac{\sum_{t > M/\epsilon} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]}{P} = \frac{\sum_{t > M/\epsilon} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]}{\sum_{t > 0} \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]}  \leq \frac{\sum_{t > M/\epsilon} t \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]}{\sum_{t > M/\epsilon} \operatorname{I}[X_t, X_0 | X_{1\dots t-1}]}< \epsilon$
%
%Let $T := M/\epsilon$.
%Then 
%\end{proof}
...
There is a tradeoff between the surprisal the listener experiences in the future, and $\operatorname{I}[Z, (X_{< 0})]$: For greater values of this mutual information, lower surprisal can be achieved in the future -- up to a certain limit where additional storage cannot further reduce surprisal. This limit corresponds to perfect modeling, and results in Crutchfield and colleagues' notion of complexity.
...
\begin{equation}\label{eq:ib}
	\max_{Z} \left( \operatorname{I}[\overrightarrow{X}, Z] - \beta \cdot \operatorname{I}[Z, (X_{<0})] \right)
\end{equation}
...
If we considered $\operatorname{H}[Z]$ as the measure of memory need, instead of $\operatorname{I}[Z, (X_{<0})]$, we would get a different tradeoff curve; however, the curve will always be bounded by the curve obtained from \ref{eq:ib}.
