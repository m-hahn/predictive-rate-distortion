...
In Section~\ref{sec:OCF}, we describe the prior method of Optimal \textcolor{red}{Causal} Filtering (OCF).
...
In Section~\ref{sec:language}, we apply NPRD to data from five \textcolor{red}{natural} languages, providing the first estimates of predictive rate-distortion of natural language.
...
After every pass through the training set, the objective (\ref{eq:bound}) is evaluated on the validation set using a Monte Carlo estimate with one sample $Z$ per trajectory; optimization terminates once the value on the \textcolor{red}{validation} set does not decrease any more.
...
\textcolor{red}{Our Equations} (\ref{eq:bound-mc-rate}-\ref{eq:bound-mc-distortion}) provide \emph{unbiased} estimators of upper bounds, whereas overfitting \emph{biases} the values obtained by OCF.
Given that \textcolor{red}{Neural Predictive Rate-Distortion} provably provides upper bounds on rate and prediction loss (up to sampling error), one can objectively compare the quality of different estimation methods:
...
We use the encoder network that computes the code vector $h$ (\ref{eq:z-gauss}) to also estimate the marginal probability of the past observation sequence, $P_\eta(\finitepast)$. $P_\eta$ has support over sequences of length $M$.
...
that the cross-entropy under $P_\eta$ of $\finitefuture$ is equal to the cross-entropy of $\finitepast$ under the same encoding distribution: $\mathbb{E}_{X_{\finitepast\finitefuture}}\left[\log P_\eta(\finitefuture)\right] = - \mathbb{E}_{X_{\finitepast\finitefuture}}\left[\log P_\eta(\finitepast)\right]$. 
...
In order to optimize parameters for estimation of $P_\eta$, we add the cross-entropy term $- \mathbb{E}_{X_{\finitepast\finitefuture}}\left[\log P_\eta(\finitepast)\right]$ to the PRD objective~(\ref{eq:bound}) during optimization, %\rljf{Why?},
...
\paragraph{\textcolor{red}{Neural Predictive Rate-Distortion}}
...
The true predictive rate-distortion curve was computed in \cite{marzen-predictive-2016} using the analytically known \textcolor{red}{$\epsilon$-machine}.
...
\section{Estimating \textcolor{red}{Predictive Rate-Distortion}   for Natural Language}\label{sec:language}
...
Predictive Rate-Distortion describes how much memory of the past humans need to maintain to predict future words accurately.
...
Neural encoders and decoders can use \textcolor{red}{the geometric} structure of the code space to encode generalizations across different dimensions, supporting a very large effective number of distinct possible codes.
...
On Even Process and Random Insertion Process, NPRD took a median of 10 minutes to process 3M training samples.
OCF took less than one minute at $M \leq 5$; however, it does not scale to larger values of $M$.
...
%\includegraphics[width=0.45\textwidth]{code/figures/even-beta-mem.pdf}
	\caption{\textcolor{red}{Rate-Distortion for the Even Process (left) and the Random Insertion Process (right), estimated using a simple diagonal unit Gaussian approximation for $q$. Gray lines: Analytical curves. Red dots: Multiple runs of NPRD ($> 200$ samples). Red line: Tradeoff curve computed from NPRD runs. Compare Figure~\ref{fig:even} for results from full NPRD. }}\label{fig:even-noFlow}
\end{figure*}
...
\definecolor{darkgreen}{rgb}{0.33, 0.6, 0.18}
...
\textbf{\textcolor{red}{- - - -}}\ \ Low rate ($\log \frac{1}{\lambda}=1.0$)\ \ \ \ \ \ 
\textbf{\textcolor{darkgreen}{-- - --}}\ \ Medium rate ($\log \frac{1}{\lambda}=3.0$)\ \ \ \ \ \ 
\textbf{\textcolor{blue}{------}}\ \ High rate ($\log \frac{1}{\lambda}=5.0$)
\end{tabular}

	\caption{\textcolor{red}{Four example outputs from English word-level modeling, with low rate ($\log \frac{1}{\lambda} = 1$; red, dotted), medium rate ($\log \frac{1}{\lambda} = 3$; green, dashed), high rate ($\log \frac{1}{\lambda} = 5$; blue, solid). For each sample, we provide the prior context $\finitepast$ (top), and the per-word cross-entropies (in nats) on the future words $\finitefuture$ (bottom).}
