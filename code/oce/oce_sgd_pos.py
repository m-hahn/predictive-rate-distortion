...
if args.language == "English_Words":
   key = "word"
   languageForCorpus = "English"
else:
   key = "posUni"
   languageForCorpus = args.language
...
backwards = ("Backwards" in languageForCorpus)
languageForCorpus = languageForCorpus.replace("Backwards", "")
from corpusIterator import CorpusIterator

ngrams = {}

lastPosUni = ("EOS",)*(2*horizon-1)
for sentence in CorpusIterator(languageForCorpus,"train", storeMorph=True).iterator():
 if backwards:
    sentence = sentence[::-1]
...
lastPosUni = ("EOS",)*(2*horizon-1)
for sentence in CorpusIterator(languageForCorpus,"dev", storeMorph=True).iterator():
 if backwards:
    sentence = sentence[::-1]
 for line in sentence:
   nextPosUni = line[key]
...
with open("/u/scr/mhahn/deps/memory-upper-neural-pos-only-discrete/estimates-"+args.language+"_"+__file__+"_model_"+str(myID)+".txt", "w") as outFile:
