...
E0=1.7
plot = ggplot(dataU, aes(x=EE, y=Memories)) + geom_point()+ theme_classic() + xlab("I[Z, Future]") + ylab("I[Z, Past]") + theme(legend.position="none") + xlim(0, NA) + ylim(0, NA) + geom_point(data=data.frame(y=y), aes(x=E0-1.6*alpha/beta * gammainc(beta, (y/alpha)^(1/beta)), y=y), color="red")
ggsave("figures/ru-words-info-fitted.pdf", plot=plot) 
