...
while True:
   ...
   dropout_rate = random.choice([0.1]) # dropout1
   ...
   model = "REAL_REAL"
   input_dropoutRate = random.choice([0.05]) #dropout2

   batchSize = random.choice([16, 32, 64])
   input_noising = 0.0
   horizon = 30
   if True:
      beta = math.exp(random.uniform(-6, -2)) #random.random() * 0.05
   else:
      ...
      if beta > 0.7 or beta < 0.005:
         continue
   print(beta)
   flow_length = random.choice([1,2,3,4]) #,4,5]) #4,5,6])
