...
while True:
   language = "PTB"
   languageCode = "en"
   dropout_rate = random.choice([0.0,0.1,0.1,0.1,0.2])
   emb_dim = 100 #random.choice([50,100,200,300])
   rnn_dim = 256 #random.choice([256,512])
   rnn_layers = random.choice([1])
   lr_lm = random.choice([0.00005, 0.0001, 0.0001,0.0001,0.0001,0.0001,0.0002])
   model = "REAL"
   input_dropoutRate = random.choice([0.0,0.1,0.1,0.1,0.2])
   ...
   subprocess.call(map(str,["./python27", "yWithMorphologySequentialStreamDropoutDev_BaselineLanguage_Fast_SaveLast_NoFinePOS_POSOnly_Variational_Bottleneck_TwoRNNs_NeuralFlow_Optimizer_DIMENSIONS_SEPARATE_WordsLR.py", language, languageCode, dropout_rate, emb_dim, rnn_dim, rnn_layers, lr_lm, model, input_dropoutRate, batchSize, horizon, beta, flow_length, flowtype]))
