...
prescribedID = sys.argv[16] if len(sys.argv)> 16 else None
...
state = torch.load("/u/scr/mhahn/CODEBOOKS/"+language+"_"+"nprd_words_PTB_saveCodebook.py"+"_code_"+prescribedID+".txt")
...
itos_total = ["EOS", "EOW", "SOS"] + itos_word[:vocab_size-1] + ["OOV"] #+ itos_lemmas[:vocab_size] + itos_morph
...
for i in range(len(components)):
   components[i].load_state_dict(state["components"][i])
 ...
 #             print(beta, flow_length, klLoss.mean(), lossesWord.mean(), beta * klLoss.mean() + lossesWord.mean() )
           ...
           #              j=0
                   ...
                   for i in range(0,len(input_indices[0])-1): #range(1,maxLength+1): # don't include i==0
                         ...
                         print >> outFile, "\t".join(map(str,(i, itos_total[input_indices[j][i+1]], lossesCPU[i][j])))
...
if True: #counter % DEV_PERIOD == 0:
          with open("../../results/nprd-samples/"+"samples_"+str(myID)+".txt", "w") as outFile:
